                ...
                {followUps.map(q=>(
                  <button key={q} className="chip" onClick={()=>{
                    ...
                    fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},
                      ...
                      .then(r=>r.json()).then(d=>{const txt=d.content?.filter(b=>b.type==="text").map(b=>b.text||"").join(" ").replace(/ +/g," ").trim();setMsgs(prev=>[...prev,{role:"assistant",content:txt||"The oracle went silent."}]);})
